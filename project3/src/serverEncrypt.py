...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as serverSocket:
    ...
    chunkList = checksums_file(File_path) #Destination path
    ...
    # send entire buffer, sendto() is only
    #  for UDP datagram
    ...
    if client_option == "download":
        ...
        #use the list that the client sent over
        # For every element inside of the list, get the chunks and get the offset
        # Index = offset/BLOCK_SIZE
        # server_list.append(index, chunk)
        # Write out every single line of chunk inside our server_list
        # To the SERVER FILE

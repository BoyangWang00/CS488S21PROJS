...
if option == 'download':
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as clientSocket:
        ...
        while True:
            ...
            print("Data is ", data)
            #Decrypt list of received boxes from server:
            data = box.decrypt(data)
            print("data decoded is:", data.decode()) # decode('utf-8')?
        ...
        old_checksums = translate_from_Json(received_data.decode()[:-2]) #old_checksums has json server old list
        ...
        new_file_list = checksums_file(new_file_name) #client's new longer hashlist of encrypted boxes
        copy_new_file_list = new_file_list.copy() # Make a copy of the new_list_file just in case
        ...
        for block in copy_new_file_list.chunks:
            ...
            if block.md5 not in [items.md5 for items in localChecksums.chunks]: #new block not in old list
                #reading the raw data of changes that old list does not contain
            ...
            else: # Remove every element from the new_list_file.copy that the server has
                copy_new_file_list.remove(checksums.__getitem__(chunk_number))

#localChecksums.remove(checksums.chunks[checksums.get_chunk(data)])

#NOT CORRECT: reading the raw data, no point in the checksumfile => take received server list,

# Compare the copy of the new_list_file to the server's received list

# Now the new_list_copy has all the elements that serer DOES NOT have
#Send the new_list_file_copy to the server
                    
                #if signature.md5 in [items.md5 for items in temp_log_list.chunks]:
        #Put list of data changes to a temp file and call checksums_file on it:
        with open('temp.txt', 'w') as f:
            for item in data_list_to_send:
                f.write("%s" % item)
            encrypted_list_to_send = checksums_file(f) #Encrypt the changes that server does not have

        #Send the data blocks and offset list to reconstruct
        to_send = {'data_list_to_send':encrypted_list_to_send, 'new_file_list.chunks':new_file_list.chunks}
...
#Make a copy of new)file lsit, if thee lisst that thee servere's list come back, check if it's in the new_file list copy,then remove matches. Send whatever elements are left to server + send the new file list to server.
#if the list that server sends over has any element that is inside our 

#in old list, append offset of the changes to the server

#In server code, client send list changes and server looks for the offset: index =offset/blocksize, list that server 

#inside list copy, make a copy of it
#Get chunk and offset from the list that clietn sent over. => when server appends, utilize index and information

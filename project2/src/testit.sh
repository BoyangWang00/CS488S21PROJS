...
while [ $i -lt 1 ]; do
    let i=i+1 
	python3 ./receiver.py 5004 > RECEIVED_FILE &
	python3 ./forwarder.py 5002 5004 &
	cat SMALL_FILE | python3 ./sender.py 127.0.0.1 5002 &
	sleep 30s
	sync
	diff RECEIVED_FILE SMALL_FILE
	if [ $? != 0 ]; then
